...
export function executeExplainDocumentIssue(args: { issue?: unknown }, context: SupportChatContext): ToolOutcome {
  ...
  const where =
    owner?.kind === "account"
      ? "They replace it themselves on their Profile, under their account documents. You can't upload anything for them."
      : owner?.kind === "guest"
        ? "They replace it themselves where they uploaded it (the identity document in the booking form). You can't upload anything for them."
